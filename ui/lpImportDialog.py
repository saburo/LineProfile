...
class LPImportDialog(QDialog, FORM_CLASS):
    ...
    def __init__(self, iface):
        ...
        QObject.connect(self.Btn_OpenFileSelectDialog, SIGNAL('clicked()'), self.getShapeFilePath)
    ...
    def checkParams(self):
        if self.RadBtn_FileSelect.isChecked() and self.TBox_ShapeFilePath.text() == '':
            return False
        return True

    def getShapeFilePath(self):
        self.shapeFileName = QFileDialog.getOpenFileName(self, 'Select a shape file', '~/', 'ESRI Shapefile (*.shp)')
        if self.shapeFileName:
            self.TBox_ShapeFilePath.setText(self.shapeFileName)
            if not self.checkShapeFileType():
                QMessageBox.information(self, 'Error', 'Selected file is not appropriate')

    def checkShapeFileType(self):
        out = True
        profileLine = self.iface.addVectorLayer(self.shapeFileName , 'tmp', 'ogr')
    ...
    def addProfileLineLayers(self):
        ...
        for layer in canvas.layers():
            # vector layer
            if layer.type() == 0 and layer.geometryType() == 1 and layer.featureCount() == 1:
